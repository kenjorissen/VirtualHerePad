...
spec = importlib.util.spec_from_file_location('touch_stop', Path(__file__).resolve().parents[1] / 'touch-stop.py')
touch = importlib.util.module_from_spec(spec)
spec.loader.exec_module(touch)


def make_hold(slots=None):
    return touch.CornerHold((100, 1100), (200, 2200), slots or {
        0: {'id': -1, 'x': 100, 'y': 200}, 1: {'id': -1, 'x': 100, 'y': 200}})
...
class GestureTests(unittest.TestCase):
    ...
    def test_capability_detection_rejects_mouse_and_accepts_direct_multitouch(self):
        ...
        def fake_ioctl(fd, number, size, initial=b''):
            ...
            info = {47: (0, 0, 1, 0, 0, 0), 53: (100, 100, 1100, 0, 0, 0),
