...
class PreflightTests(unittest.TestCase):
    ...
    def test_probes_leave_no_files_or_directories(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(['bash', '-c', self.block(folder)], capture_output=True, text=True, timeout=3)
    ...
    def test_unwritable_path_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(['bash', '-c', self.block(folder)], env=env, capture_output=True, text=True, timeout=3)
    ...
    def test_metadata_without_git_checkout(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(['bash', '-c', block], cwd=folder, env=env,
                                    capture_output=True, text=True, timeout=3)
            self.assertEqual(result.returncode, 0, result.stderr)
            info = (folder / 'build-info.txt').read_text()
            self.assertIn('VHP_COMMIT=unknown\n', info)
            self.assertIn('VIRTUALHERE_SHA256=' + hashlib.sha256(b'test binary fixture').hexdigest(), info)
