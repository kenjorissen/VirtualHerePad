    ...
    def feed(self, kind, code, value, now):
        ...
        if kind == EV_ABS:
            ...
            if code == ABS_MT_SLOT:
                ...
                field = {ABS_MT_TRACKING_ID: 'id', ABS_MT_POSITION_X: 'x', ABS_MT_POSITION_Y: 'y'}.get(code)
                if field:
...
def wait_timeout(devices, next_scan, now):
    ...
    deadlines = [device.hold.since + HOLD_SECONDS for device in devices
        ...
        while True:
            if not devices and time.monotonic() >= next_scan:
                for path in sorted(Path('/dev/input').glob('event*')):
                    ...
                    devices[path] = device
                    warned_missing = False
                    print(f'Touch exit monitoring {path}: hold one finger in any corner for 2 seconds.', flush=True)
                if not devices and not warned_missing:
                    print('No supported touchscreen found; retrying. Use local-keyboard Ctrl+C to exit.', flush=True)
                    warned_missing = True
            ...
            for path, device in list(devices.items()):
                try:
                    ...
                    if device.expired():
                        ...
                        print('Touchscreen corner hold detected; requesting VHP shutdown.', flush=True)
                        return
